// 'Alternative search bar legacy' script for Firefox 153+ by Aris
//
...
// ----------------------------------------------------------------------
// LEGACY VERSION
// Requires the old searchbar to be enabled:
...
var AltSearchbar = {
  init: async function() {
    ...
    const SS = Services.search ?? lazy.SearchService;
    ...
    try {
      ...
      function selectEngineByScrollingOverButton() {
        if (!old_search_engine_selection_popup) SS.getVisibleEngines().then(engines => searchbar.engines = engines);
          ...
          try {
            ...
            try {
              ...
              engines.forEach((engine, index) => {
                ...
                menuitem.addEventListener("command", () => {SS.addOpenSearchEngine(engine.uri, engine.icon);});
        ...
        if (revert_to_first_engine_after_search && searchbar.doSearch) {
          setTimeout(function() {
            ...
            if (!old_search_engine_selection_popup && !select_engine_by_scrolling_over_button)
              SS.getVisibleEngines().then(engines => searchbar.engines = engines);
